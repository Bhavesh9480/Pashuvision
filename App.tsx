...
const AppContent: React.FC = () => {
  ...
  useEffect(() => {
    dbService.getAllRegistrations().then(data => {
        ...
        setIsDbLoading(false);
        alert("Could not load local data. The app may not function correctly.");
    });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-cream-50 text-primary-900 font-sans">
      <Header onSearch={handleGlobalSearch} showSearch={currentView === 'DASHBOARD'} isOnline={isOnline} isScrolled={isScrolled} onOpenGuide={() => setIsGuideOpen(true)} />
