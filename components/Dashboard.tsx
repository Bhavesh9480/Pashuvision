...
import { Registration, ChatMessage, BreedInfo, AnimalResult } from '../types';
...
const HealthHubModal: React.FC<{ 
    ...
    const FilterButton: React.FC<{days: number, text: string}> = ({ days, text }) => (
      <button 
        onClick={() => setFilterDays(days)}
        className={`px-3 py-1 text-sm rounded-full transition-colors ${filterDays === days ? 'bg-secondary-600 text-white font-semibold' : 'bg-cream-100 text-primary-700 hover:bg-cream-200'}`}
      >
        {text}
      </button>
    );
                ...
                <div className="p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <FilterButton days={7} text={t('dashboard.healthHub.filter7')} />
                      <FilterButton days={30} text={t('dashboard.healthHub.filter30')} />
                      <FilterButton days={90} text={t('dashboard.healthHub.filter90')} />
                    </div>
                  </div>
                </div>
