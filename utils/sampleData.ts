...
export const generateSampleRegistrations = (): Registration[] => {
    ...
    // Generate 5 "user" records, now marked as samples.
    // The "Recent Registrations" carousel will only show truly new registrations.
    for (let i = 0; i < 5; i++) {
        const owner = generateOwner();
        const animalCount = 1;
        const animals = Array.from({ length: animalCount }, generateAnimal);
        ...
        registrations.push({
            id: `reg-${new Date(timestamp).getTime()}-user-${i}`,
            timestamp,
            owner,
            animals,
